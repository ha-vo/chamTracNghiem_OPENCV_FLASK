...
img = cv2.imread("/home/havo/autograding/wep-app-autograding/phieu4.jpg")
width = 550
...
img_canny = cv2.Canny(img_blur,0,350)
img_erosion = cv2.erode(img_blur,kernel,iterations=1)
img_test = cv2.Canny(img_erosion,100,500)
img_dilation = cv2.dilate(img_test, kernel, iterations=1)
img2 = img.copy()
cv2.imshow("t3",img_erosion)
cv2.imshow("t2",img_test)
cv2.imshow("t1",img_dilation)
contour,_ = cv2.findContours(img_test,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_NONE)
list_contour = []
for con in contour:
